...
def main():
	...
	df = pd.DataFrame()
	df['Time'] = df_read['time'].iloc[:3000]
	df['keff'] = df_read['keff'].iloc[:3000]
	df['cr_pos'] = df_read['rod_position'].iloc[:3000]
	#print(df.head())
	...
	df_test['Time'] = df_read['time'].iloc[3000:4000]
	df_test['keff'] = df_read['keff'].iloc[3000:4000]
	df_test['cr_pos'] = df_read['rod_position'].iloc[3000:4000]
	...
	#K=X[:,98,1]
	cr_data = df_test['cr_pos'][27:1000]
	plt.plot(np.arange(0,len(cr_data)),cr_data,label='True data')
