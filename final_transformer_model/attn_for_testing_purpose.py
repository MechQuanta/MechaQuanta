...
def main():
	...
	df_test['Time'] = df_read['time'].iloc[3000:3500] #+noise
	df_test['keff'] = df_read['keff'].iloc[3000:3500] #+noise
	df_test['cr_pos'] = df_read['rod_position'].iloc[3000:3500] #+noise
	...
	print('y before= ',y)
	#predictions = scaler.inverse_transform(df[['Time','keff','cr_pos']].values)[:,2]
	predictions = scaler2.inverse_transform(predictions)
	y_reshaped = y.reshape(-1, 1)  # Shape: (n_samples, 1)
	dummy_array = np.zeros((y_reshaped.shape[0], 3))  # Shape: (n_samples, 3)
	dummy_array[:, 2] = y_reshaped.ravel()
	y_inverse = scaler_dummy.inverse_transform(dummy_array)[:, 2] 
	#y_corr = scaler_dummy.inverse_transform(df_test[['Time','keff','cr_pos']].values)[:,2]
	print('y after = ',y_inverse)
	plt.figure(figsize=(10,6))
	#K=X[:,98,1]
	#cr_data = df_test['cr_pos'][27:1001]
